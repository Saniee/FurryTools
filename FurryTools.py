...
siteCheckbox = tk.Checkbutton(mpdFrame, text="Download NSFW?", variable=nsfwMPD, onvalue=True, offvalue=False)
siteCheckbox.grid()

HFDL1 = tk.Label(mpdFrame, text="Input a url without anything else then the ID (https://e926.net/posts/XXXXXXX)")
HFDL1.grid()
inp = tk.Entry(mpdFrame)
inp.grid()
setFileBtn = tk.Button(mpdFrame, command= lambda: mpd.getTxtPath(inp) ,text="Click to get a txt file with urls instead")
setFileBtn.grid()
HFDL3 = tk.Label(mpdFrame, text="Click to set the download directory.")
HFDL3.grid()
dirBtn = tk.Button(mpdFrame, command= lambda: mpd.getDirPath(dirLabel), text="Set directory!")
dirBtn.grid()
dirLabelTxt = tk.Label(mpdFrame, text="Downloading to:")
dirLabelTxt.grid()
dirLabel = tk.Label(mpdFrame, text=f"{os.getcwd()}")
dirLabel.grid()
runBtn = tk.Button(mpdFrame, command= lambda: mpd.runUrl(inp.get(), dirLabel["text"], nsfwMPD.get()), text="Download!")
runBtn.grid()

tagSearchFrame = tk.Frame(app, width=250, height=250)
tagSearchFrame.grid(row=0, column=1)

siteCheckboxFV = tk.Checkbutton(tagSearchFrame, text="Download NSFW?", variable=nsfwTag, onvalue=True, offvalue=False)
siteCheckboxFV.grid()
...
limitEntry.grid()
runBtn = tk.Button(tagSearchFrame, text="Search & Download!", command= lambda: mpd.runSearch(searchEntry.get(), limitEntry.get(), nsfwTag.get()))
runBtn.grid()

fvFrame = tk.Frame(app, width=250, height=250)
fvFrame.grid(row=0, column=2)

siteCheckboxFV = tk.Checkbutton(fvFrame, text="Download NSFW?", variable=nsfwFV, onvalue=True, offvalue=False)
siteCheckboxFV.grid()
...
runBtn = tk.Button(master=fvFrame, command= lambda: fv.run(nameInput.get(), limitInput.get(), nsfwFV.get()), text="Download!")
runBtn.grid()
